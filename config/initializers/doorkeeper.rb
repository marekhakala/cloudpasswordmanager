Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |routes|
    ...
    user = User.find_for_database_authentication(email: uid)
    # Only accept if we have a valid application, user and valid credentials
    user if user && user.valid_password?(secret)
  end
  ...
  #   Admin.find_by_id(session[:admin_id]) || redirect_to(new_admin_session_url)
    if not current_user.nil? and not current_user.role.nil? and (current_user.role.code == 'ADMIN1' || current_user.role.code == 'SUPERADMIN1')
      current_user
    else
  ...
  realm "CloudPasswordManager (Doorkeeper)"
end
